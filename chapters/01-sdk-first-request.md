...
Notice the type guard that filters and narrows in one move, and the reuse of the SDK's own `Anthropic.Message` type. The `stop_reason` values you meet here:

- ✅ `end_turn` - finished naturally, the healthy case.
- ✂️ `max_tokens` - the cap cut the reply off; raise it if you did not want truncation.
- 🛑 `stop_sequence` - a stop string you configured was emitted.

(`tool_use` and `refusal` are among the rest you meet later.) The two `usage` counts - `input_tokens` for everything you sent, `output_tokens` for what came back - are how you reason about cost and, eventually, context size.
...
::: info Bun caveat
A shell script does not get `.env` auto-loaded the way a `.ts` file does, so export your variables first.
:::

## 🎛️ Pick a model and understand max_tokens
...
- ⚖️ `claude-sonnet-4-6` - the balanced default.
...
This is the place to nail down `max_tokens`: **it is an upper bound on the response length, not a target.** `max_tokens: 128` allows at most 128 tokens, it does not request them. Finish in 20 and you get a 20-token reply with `stop_reason: 'end_turn'`; run out of room and the reply is truncated with `stop_reason: 'max_tokens'`. Pick a value that fits the answer you expect, and watch `stop_reason` to catch a cap set too low.
...
➡️ One `client.messages.create` call works against Anthropic direct or any compatible gateway - switching is configuration, not a rewrite.
